...
meanval = 162.0             # Specify mean of distribution.
sdval = 15               # Specify standard deviation of distribution.
xlow  = meanval - 5*sdval # Specify low end of x-axis.
xhigh = meanval + 5*sdval # Specify high end of x-axis.
...
plot( x , y , type="h" , lwd=1 , cex.axis=1.5
	...
	, main="Women's Heights in cm" , cex.main=1.5 )
lines( x , y )
...
text( sdval , .9*max(y) , bquote( paste(Delta , "x = " ,.(dx))), adj=c(1,1))
text( sdval , .8*max(y) ,
